...
void lstm(
	...
	float recurrent_kernel[50][NEURONS * GATES], //shape=(self.units, self.units * 4)
	float recurrent_kernel2[50][NEURONS * GATES], //shape=(self.units, self.units * 4)
	float recurrent_kernel3[50][NEURONS * GATES], //shape=(self.units, self.units * 4)
	float recurrent_kernel4[50][NEURONS * GATES], //shape=(self.units, self.units * 4)
    ...
    )
{
    ...
    for (i = 0; i < 25; i++) {
            for (j = 0; j < NEURONS; j++) {
                ...
                i_state[j] += (recurrent_kernel2[i][j + NEURONS * 0] * h_state[k]);
    	    	f_state[j] += (recurrent_kernel2[i][j + NEURONS * 1] * h_state[k]);
    	    	c_state[j] += (recurrent_kernel2[i][j + NEURONS * 2] * h_state[k]);
    	    	o_state[j] += (recurrent_kernel2[i][j + NEURONS * 3] * h_state[k]);
    ...
    int l = 50;
        for (i = 0; i < 25; i++) {
                for (j = 0; j < NEURONS; j++) {
                    ...
                    i_state[j] += (recurrent_kernel3[i][j + NEURONS * 0] * h_state[l]);
        	    	f_state[j] += (recurrent_kernel3[i][j + NEURONS * 1] * h_state[l]);
        	    	c_state[j] += (recurrent_kernel3[i][j + NEURONS * 2] * h_state[l]);
        	    	o_state[j] += (recurrent_kernel3[i][j + NEURONS * 3] * h_state[l]);
        ...
        int m = 75;
                for (i = 0; i < 25; i++) {
                        for (j = 0; j < NEURONS; j++) {
                            ...
                            i_state[j] += (recurrent_kernel4[i][j + NEURONS * 0] * h_state[m]);
                	    	f_state[j] += (recurrent_kernel4[i][j + NEURONS * 1] * h_state[m]);
                	    	c_state[j] += (recurrent_kernel4[i][j + NEURONS * 2] * h_state[m]);
                	    	o_state[j] += (recurrent_kernel4[i][j + NEURONS * 3] * h_state[m]);
